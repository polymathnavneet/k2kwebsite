...
export function Games() {
  ...
  useEffect(() => { if (openCards.length !== 2) return; setMoves(value => value + 1); const timer = setTimeout(() => { if (openCards[0].pairIndex === openCards[1].pairIndex) setMatched(value => [...value, openCards[0].pairIndex]); setOpen([]); }, 550); return () => clearTimeout(timer); }, [openCards]);
