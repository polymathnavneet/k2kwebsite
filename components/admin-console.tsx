...
import { ArrowUp, Download, LocateFixed, Plus, RefreshCw, Trash2 } from "lucide-react";
...
export function AdminConsole() {
  ...
  useEffect(() => { const saved = localStorage.getItem("alw-admin-token"); if (saved) setToken(saved); }, []);
  ...
  function useGps() {
    ...
    navigator.geolocation.getCurrentPosition(position => {
      setJourney(value => ({ ...value, lat: position.coords.latitude, lon: position.coords.longitude }));
      setStatus(`GPS ready · about ${Math.round(position.coords.accuracy)} m accuracy.`);
  ...
  return <section className="admin-app">
    <div className="admin-bar"><div><b>A LONG WALK</b><span>{status}</span></div><Button variant="outline" onClick={loadAll}><RefreshCw /> Refresh</Button><Button variant="outline" onClick={() => { localStorage.removeItem("alw-admin-token"); setConnected(false); setToken(""); }}>Change passcode</Button></div>
